...
function jumpSeminar(id1) {
  //jump to specific Seminar
  let info = id1;
  let arr = info.split(";");
  let sid = arr[0];
  let groupingmethod = arr[1];
  updateCookie("seminarCurrent", sid);
  updateCookie("groupingmethodCurrent", groupingmethod);
  window.location.href =
    "/student/course/" +
    getCookie("coursecurrent") +
    "/seminar/" +
    sid +
    "?type=" +
    groupingmethod;
...
function jumpTopic(id1) {
  //jump to specific topicF
  let tid = id1;
  ...
  window.location.href =
    ...
    getCookie("coursecurrent") +
    "/seminar/" +
    getCookie("seminarCurrent") +
    "/topic?type=" +
    getCookie("groupingmethodCurrent");
}
...
function courseinfo() {
  ...
  $.ajax({
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        let str = "";
        for (let i = 0; i < data.seminarList.length; i++) {
          str +=
            '<div class="block" id="' +
            data.seminarList[i].seminarId +
            ";" +
            data.seminarList[i].groupingMethod +
            '" onclick="jumpSeminar(this.id)"><div class="blockFont">' +
            data.seminarList[i].name +
            "</div></div>";
        }
        ...
        let fixg =
          '<div class="block" id="' +
          data.fixGroup +
          '" onclick="jumpGroup(this.id)""><div class="blockFont">固定分组</div></div>';
  ...
  //     success: function (data,status,xhr){
  //         if(xhr.status === 200){
  //             let seminar=document.getElementById("seminarinfo");
  //             let contenthead = '<div class="title">讨论课</div>'+
...
function seminarinfoF() {
  //show seminar info(fixed)
  $.ajax({
    type: "get",
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        if (data.groupingMethod === "fixed") $("#groupingmethod").html("固定");
        ...
        for (let i = 0; i < data.topics.length; i++) {
          str +=
            '<div class="smallblock"><div class="blockFont" onclick="jumpTopic(this.id)" id="' +
            data.topics[i].id +
            '">' +
            data.topics[i].name +
            "</div></div>";
...
function seminarinfoR() {
  //show seminar info(random)
  $.ajax({
    type: "get",
    url: "/seminar/" + getCookie("seminarCurrent"),
    dataType: "json",
  ...
  $.ajax({
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        //let teaminfo = document.getElementById("randomteam");
        let str1 = "";
        let str2 = "";
        for (let i = 0; i < data.topics.length; i++) {
          str1 +=
            '<label class="itemName" id="topicselected">' +
            data.topics[i].name +
            "</label><br/>";
        }
        ...
        let teaminfohead =
          ...
          '<label class="itemName" id="groupnum">' +
          data.id +
          "</label><br/>" +
          '<label class="itemName">组长：</label>' +
          '<label class="itemName" id="groupleader">' +
          data.leader.name +
          "</label><br/>" +
          '<label class="itemName">组员：</label>';
        for (let i = 0; i < data.members.length; i++) {
          str2 +=
            '<label class="itemName">' + data.members[i].name + "</label>";
        }
  ...
  $.ajax({
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        select.innerHTML =
          '<div class="modifyButton" onclick="selecttopic(this.id)" id="' +
          data.id +
          '"> 选择话题 </div>';
      }
    },
  ...
  $.ajax({
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        let tablehead = "<tr><th>角色</th><th>学号</th><th>姓名</th></tr>";
        let leader =
          "<tr><td>队长</td><td>" +
          data.leader.number +
          "</td><td>" +
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        if (isLeader(data.leader.id)) {
          opr = "<th>操作</th>";
          icon =
            '<td><img src="/img/home.png" id = " ' +
            data.leader.id +
            '" onclick="return groupinforemove(this.id);"></td>';
        }
        let tablehead =
          "<tr><th>角色</th><th>学号</th><th>姓名</th>" + opr + "</tr>";
        let leader =
          "<tr><td>队长</td><td>" +
          data.leader.number +
          "</td><td>" +
  ...
  $.ajax({
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        let tablehead = "<tr><th>学号</th><th>姓名</th>" + opr + "</tr>";
        for (let i = 0; i < data.length; i++) {
          if (getCookie("isLeader") === true) {
            if (i % 2 === 0) {
              str +=
                ...
                '<td><img src="/img/home.png" id = " ' +
                data[i].id +
                '" onclick="return groupinfoadd(this.id);"></td></tr>';
            } else {
              str +=
                '<tr class="alt"><td>' +
                data[i].number +
...
function gradeinfo() {
  //StudentViewGroupPage  showgroup grade list
  $.ajax({
    type: "get",
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        let str = "";
        let tablehead =
          "<tr><th>讨论课</th><th>组名</th><th>组长</th><th>课堂讨论课得分</th><th>报告分数</th><th>总分</th></tr>";
        for (let i = 0; i < data.length; i++) {
          if (i % 2 === 0)
            str +=
              "<tr><td>" +
