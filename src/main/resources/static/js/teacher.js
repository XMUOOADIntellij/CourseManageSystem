    ...
    $.ajax({
        ...
        success: function(data, textStatus, xhr) {
            if (xhr.status === 200) {
                ...
                var content=document.getElementById(roundId);   //获取外围容器
                var str="";
                $.each(data, function(i, item) {
                    console.log(item);
                ...
                content.innerHTML=str;
                $.each(data, function(i, item) {
                    getTeamDetailScoreByRound(roundId,item.id);
                });
