...
function jumpcourse(cid) {
  //点击修改课程时，将课程id号存储在cookie中
  updateCookie("courseCurrent", cid);
  window.location.href = "/teacher/course/" + cid + "/update"; //页面跳转
}

function jumpcoursedetail(cid) {
  //教师课程列表页面点击课程蓝框->跳转到某个具体课程页面时调用，用cookie存课程Id
...
function jumpclassdetail(cid) {
  ...
  window.location.href =
    "/teacher/course/" + getCookie("courseDetail") + "/class/" + cid; //进入班级详情页面
}
...
function jumpseminardetail(cid) {
  ...
  window.location.href =
    "/teacher/course/" + getCookie("seminarDetail") + "/seminar/" + cid; //进入讨论课详情页面
}
...
function courselist() {
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
      if (xhr.status === 200) {
        let content = document.getElementById("coursecontent"); //获取外围容器
        let str = "";
        str += '<div class="title">课程信息</div><hr class="line"/>';
        for (let i = 0; i < data.length; i++) {
          str +=
            '<div class="main_box_right_content"><h3 class="classtitle" id="name">' +
            ' <span id="course">' +
            data[i].name +
            '</span><button type="submit"  onclick="deletecourse(' +
            data[i].id +
            ')">删除课程</button>' +
            '<button id="' +
            data[i].id +
            '"onclick="jumpcourse(this.id)  " >修改课程</button></h3>' +
            '<div class="divideline"></div><div  class="classinfo"  id="' +
            data[i].id +
            '" onclick="jumpcoursedetail(this.id) " ><table class="table"> <tr>' +
            ...
            '<td class="tabletext">班级数：<span id="numClass">' +
            data[i].numClass +
            "</span></td> " +
            '</tr><tr><td class="tabletext" id="startTime">开始时间：' +
            data[i].startTime +
            '</td><td class="tabletext" id="endTime">结束时间:' +
            data[i].endTime +
            "</td></tr></table></div></div>";
        }
        str +=
          '<div class="main_box_right_content"  onclick="window.location.href=\'/teacher/courses/create\'"><img class="addcourse" src="/img/add.png" ></div>';
...
function getcourseinfo() {
  //get course information when updating it
...
function classlist() {
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
      if (xhr.status === 200) {
        // alert("获取成功");
        let content = document.getElementById("classcontent"); //获取外围容器
        let str = "";
        str +=
        ...
        str +=
          ...
          getCookie("courseDetail") +
          '/class/create\'"><img class="addImg" src="/img/smalladd.png" alt="添加" ></div>';
        content.innerHTML = str;
      }
    },
    statusCode: {
...
function seminarlist() {
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
      if (xhr.status === 200) {
        // alert("获取成功");
        let content = document.getElementById("seminarcontent"); //获取外围容器
        let str = "";
        str +=
          '<div class="title">讨论课</div><div class="line"></div><div class="blockBody">';
        for (let i = 0; i < data.length; i++) {
          // if(i===0){
          ...
          str +=
            '<div class="block"  id="' +
            data[i].id +
            '" onclick="jumpseminardetail(this.id)"><div class="blockFont">' +
            data[i].name +
            "</div></div>";
          // if(i===data.length-1){
        ...
        str +=
          '<div class="block" onclick="window.location.href=\'/teacher/course/' +
          getCookie("courseDetail") +
          '/seminar/create\'"><img class="addImg" src="/img/smalladd.png" alt="添加" ></div>';
        content.innerHTML = str;
      }
    },
    statusCode: {
...
$(function() {
  //选择上课时间select级联显示(class_update与class_create页面都用到这个)
...
function deleteclass() {
  let ata = { id: getCookie("classDetail") };
  $.ajax({
    type: "delete",
    url: "/class/" + getCookie("classDetail"),
    data: JSON.stringify(ata),
...
function judge_end(end_time) {
  let endTime = new Date(end_time); //把字符串转换成日期格式
  let currentTime = new Date(); //获取系统时间
...
function gettopiclist() {
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
      if (xhr.status === 200) {
        // alert("获取成功");
        let content = document.getElementById("topiccontent"); //获取外围容器
        let str = "";
        for (let i = 0; i < data.length; i++) {
          str +=
            '<div class="topicBlock" id="' +
            data[i].id +
            '" onclick="jumptopic(this.id);"><div class="topicBlockFont">' +
            data[i].serial +
            "</div></div>";
        }
        str +=
          ' <div class="topicBlock" onclick="window.location.href=\'/teacher/course/' +
          getCookie("courseDetail") +
          "/seminar/" +
          getCookie("seminarDetail") +
          '/topic/create\'"><img class="addImg" src="/img/smalladd.png" alt="添加"></div></div>';
        content.innerHTML = str;
      }
    },
    statusCode: {
...
function deleteseminar() {
  let ata = { id: getCookie("seminarDetail") };
  $.ajax({
    type: "delete",
    url: "/seminar/" + getCookie("seminarDetail"),
    data: JSON.stringify(ata),
...
function updatetopic() {
  window.location.href =
    ...
    getCookie("seminarDetail") +
    "/topic/" +
    getCookie("topicDetail") +
    "/update";
}
function deletetopic() {
  let ata = { id: getCookie("topicDetail") };
  $.ajax({
    type: "delete",
    url: "/topic/" + getCookie("topicDetail"),
    data: JSON.stringify(ata),
...
function getreportlist() {
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
      if (xhr.status === 200) {
        // alert("获取成功");
        let content = document.getElementById("reportcontent"); //获取外围容器
        let str = "";
        for (let i = 0; i < data.length; i++) {
          str +=
            "  <tr>\n" +
            ...
            "</td>\n" +
            "                    <td>\n" +
            '                         <img src="/img/view.png" alt="预览" name=\'report\' id=' +
            data[i].groupName +
            " onclick='jumpreportdetail(this.id)'>\n" +
            '                        <img src="/img/download.png" alt="下载" name=\'download\' id=' +
            data[i].groupName +
            " onclick='jumpreportdetail(this.id)'>\n" +
            "                    </td>\n" +
            "                </tr>";
...
function find_groupinseminar() {
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
      if (xhr.status === 200) {
        // alert("获取成功");
        let content = document.getElementById("groupinseminar");
        let str = "";
        for (let i = 0; i < data.length; i++) {
          str +=
            " <tr>" +
