...
function jumpSeminar(id1){//jump to specific Seminar
    let info = id1;
    let arr = info.split(";");
    let sid = arr[0];let groupingmethod = arr[1];
...
function courseinfo(){//show CourseInformation
        ...
        success: function (data,status,xhr){
            if(xhr.status === 200){
                ...
                let fixg = '<div class="block" id="'+data.fixGroup+'" onclick="jumpGroup(this.id)""><div class="blockFont">固定分组</div></div>';
...
function seminarinfoR(){//show seminar info(random)
        ...
        success: function (data,status,xhr){
            if(xhr.status === 200){
                ...
                let teaminfohead = '<label class="itemName">选择话题：</label>'+str1+
...
function groupinfo(){// showgroup list
        ...
        success: function (data,status,xhr){
            if(xhr.status === 200){
                ...
                let leader = '<tr><td>队长</td><td>'+data.leader.number+'</td><td>'+data.leader.name+'</td></tr>';
                if(data.member!=null) {
...
function groupmodinfo(){// showgroup list
        ...
        success: function (data,status,xhr){
            if(xhr.status === 200){
                ...
                let tablehead = '<tr><th>角色</th><th>学号</th><th>姓名</th>'+opr+'</tr>';
                let leader = '<tr><td>队长</td><td>'+data.leader.number+'</td><td>'+data.leader.name+'</td>'+icon+'</tr>';
                if(data.member!=null) {
                    for (let i = 0; i < data.member.length; i++) {
                        if (i % 2 != 0) {
...
function gradeinfo(){//StudentViewGroupPage  showgroup grade list
        ...
        success: function (data,status,xhr){
            if(xhr.status === 200){
                ...
                $("#coursename").html(getCookie("coursename"));
                let str = "";
                let tablehead = '<tr><th>讨论课</th><th>组名</th><th>组长</th><th>课堂讨论课得分</th><th>报告分数</th><th>总分</th></tr>';
                for(let i=0;i<data.length;i++){
                    if(i%2===0)
