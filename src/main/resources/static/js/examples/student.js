...
function jumpCourse(id){
    let info = id;
    let arr = info.split(";");
    let classid = arr[0];let courseid = arr[1];
...
function classlist(){//StudenCourseSelectPage classlist
        ...
        success: function (data,status,xhr) {
            if(xhr.status === 200){
                let content=document.getElementById("classcontent");
                let str='';
                let contenthead = '<div class="title">选择课程</div><hr class="line"/><div class="checkcourse">任课教师：<input type="text" name="teacher" value = ""><br/>课程名称：<input type="text" name="course"  value = ""><input type="submit" value="查询" onclick="return classlistsearch();">';
                for(let i=0;i<data.length;i++){
