...
function stubind() {
  //StudentbindPage bindstu-updatestuinfo
...
function stumodinfo() {
  //StudentInfoModifyPage getstuinfo
  $.ajax({
    type: "get",
    url: "/me",
    dataType: "json",
...
function stuinfomod() {
  //StudentInfoModifyPage updatestuinfo
  let ata = {
    name: $("#name").val(),
    gender: $("#gender").val(),
    school: $("#school").val(),
...
function classinfo() {
  //StudentCourse_List showclassinfo
  $.ajax({
    type: "get",
    url: "/course/student",
    dataType: "json",
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        for (let i = 0; i < data.length; i++) {
          str +=
            '<div class="main_box_right_content" ><h3 class="classtitle"><span id="course">' +
            data[i].courseName +
            '</span><button id="' +
            data[i].classId +
...
function classlist() {
  //StudenCourseSelectPage classlist
  $.ajax({
    type: "get",
    url: "/class",
    dataType: "json",
    ...
    success: function(data, status, xhr) {
      if (xhr.status === 200) {
        ...
        let str = "";
        let contenthead =
          '<div class="title">选择课程</div><hr class="line"/><div class="checkcourse">任课教师：<input type="text" name="teacher" value = ""><br/>课程名称：<input type="text" name="course"  value = ""><input type="submit" value="查询" onclick="return classlistsearch();">';
        for (let i = 0; i < data.length; i++) {
          str +=
            '</div><div class="main_box_right_content"><h3 class="classtitle">' +
            data[i].courseName +
            '<button id="' +
            data[i].id +
            '" onclick="selectclass(this.id)">选择课程</button></h3>' +
            '<div class="divideline"></div><div  class="classinfo"><table class="table"><tr>' +
            '<td class="tabletext">班级：<span>' +
            data[i].name +
            '</span></td><td class="tabletext">课程地点：' +
            data[i].site +
            "</td></tr><tr>" +
...
function selectclass(id1) {
  //StudenCourseSelectPage selectclass
  let ata = { id: id1 };
  $.ajax({
    type: "post",
...
function classlistsearch() {
  //StudenCourseSelectPage classlist(searched)
  let course;
  let teacher;
