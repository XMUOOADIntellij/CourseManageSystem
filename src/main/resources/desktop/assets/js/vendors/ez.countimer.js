...
(function($, window, document, undefined) {
  ...
  var Plugin = function(element, options) {
    ...
    var format = function(time, withSeparator, overrideZeros) {
      var finalZeros = overrideZeros
        ? overrideZeros
        : plugin.options.leadingZeros;
    ...
    var mainTimer = function(omitRefresh) {
      ...
      var expressions = {
        toSeconds: function() {
          ...
          var formatted = format(withoutFormat).concat(
            plugin.options.secondIndicator
          );
        ...
        full: function() {
          ...
          var minutes = format(minutesNf.minutes, true).concat(
            format(minutesNf.seconds, false, 2)
          );
          minutes = minutes.concat(plugin.options.secondIndicator);
          return plugin.options.useHours
            ? getObjExpression(normalNf, normal)
            : getObjExpression(minutesNf, minutes);
        }
      };
      ...
      if (omitRefresh || refreshConditions[refreshMode]) {
        ...
        hasValueAttr
          ? $elm.val(finalValue.formatted)
          : $elm.text(finalValue.formatted);
      }
      // Find events to fire up
      eventHandler(refreshConditions);
      // Update moment duration
      plugin.duration = moment.duration(
        plugin.duration.asSeconds() + 1,
        "seconds"
      );
    };
    ...
    var init = function() {
      ...
      plugin.options.displayMode = fixModeValue(
        plugin.options.displayMode,
        displayMode.MAX_INDEX
      );
      ...
      refreshMode =
        plugin.options.displayMode <= displayMode.IN_HOURS
          ? plugin.options.displayMode
          : displayMode.IN_SECONDS;
      // Start?
  ...
  $.fn[pluginName] = function(methodOrOptions) {
    var method =
      "string" === typeof methodOrOptions ? methodOrOptions : undefined;
    ...
    // It's not a method
    var options =
      "object" === typeof methodOrOptions ? methodOrOptions : undefined;
